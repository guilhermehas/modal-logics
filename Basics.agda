...
data Cx : Types → Set where
  · : Cx T
  _,_ : (Γ : Cx T) (A : Ty T) → Cx T

private variable
  Γ Γ' Γ'' Δ : Cx T
  A B : Ty T

data _∈_ : (A : Ty T) (Γ : Cx T) → Set where
  top : A ∈ (Γ , A)
  pop : (i : A ∈ Γ) → A ∈ (Γ , B)
...
swap-out : (Δ Γ : Cx T) (A : Ty T) → (Δ , A) ++ Γ ⊆ (Δ ++ Γ) , A
swap-out Δ · A x = x
swap-out Δ (Γ , B) A x = swap-last (subsetdef x (weakboth (swap-out Δ Γ A)))

swap-in : (Δ Γ : Cx T) (A : Ty T) → (Δ ++ Γ) , A ⊆ (Δ , A) ++ Γ
...
swap-in-out : (Δ Δ' : Cx T) (A B : Ty T) → (Δ , A ++ Δ') , B ⊆ (Δ , B ++ Δ') , A
